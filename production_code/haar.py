...
class Haar():
    ...
    def __forward_no_normalization(self, v: list) -> dict:
        D = {}
        while (len(v) > 1):
    ...
    def __normalize_coefficients(self, n: int, coeffs: dict) -> dict:
    ...
    def forward(self, v: list) -> dict:
        n = len(v)
        return self.__normalize_coefficients(n, self.__forward_no_normalization(v))

    def suppress(self, D: dict, threshold: float) -> dict:
        return {k:(v if abs(v) > threshold else 0.0) for (k, v) in D.items()}

    def sparsity(self, D: dict) -> float:
        num_non_zeros = sum([1 if abs(x) > 0.00001 else 0 for x in D.values()])
        return num_non_zeros / len(D)

    def __unnormalize_coefficients(self, n: int, D: dict) -> dict:
    ...
    def __backward_no_normalization(self, D: dict) -> list:
        n = len(D)
        v = [D[(0,0)]]
        while len(v) < n:
            b_ix = len(v)
    ...
    def backwards(self, D: dict) -> list:
        n = len(D)
        return self.__backward_no_normalization(self.__unnormalize_coefficients(n, D))