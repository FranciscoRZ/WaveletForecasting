...
class Haar(object):
    ...
    def waveletScaleCoefficients(self, j: int) -> np.array:
        ''' Formula page 65
        '''
    ...
    def waveletScaleApprox(self, j: int) -> np.array:
        ''' Formula not in course material
        '''
        try:
            coeff = self.__coeffDict[str(j)]
        except(KeyError):
            self.__coeffDict[str(j)] = self.waveletScaleCoefficients(j)
            coeff = self.__coeffDict[str(j)]
